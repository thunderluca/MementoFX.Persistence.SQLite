...
namespace System.Linq.Expressions
{
    ...
    {
        public static SqlExpression ToSqlExpression<T>(this Expression<Func<T, bool>> expression)
        {
            if (expression == null)
            {
        ...
        private static SqlExpression Recurse(ref int i, Expression expression, bool isUnary = false, string prefix = null, string postfix = null)
        {
            if (expression is UnaryExpression unary)
            {
                return SqlExpression.Concat(unary.NodeType.ToSqlString(), Recurse(ref i, unary.Operand, true));
            }

            if (expression is BinaryExpression body)
            {
                return SqlExpression.Concat(Recurse(ref i, body.Left), body.NodeType.ToSqlString(), Recurse(ref i, body.Right));
            }
            ...
            if (expression is MemberExpression member)
            {
                ...
                if (member.Member is PropertyInfo property)
                {
                    ...
                    {
                        return SqlExpression.Concat(Recurse(ref i, expression), "=", SqlExpression.WithSingleParameter(i++, true));
                    }
            ...
            if (expression is MethodCallExpression methodCall)
            {
                ...
                {
                    return SqlExpression.Concat(Recurse(ref i, methodCall.Object), "LIKE", Recurse(ref i, methodCall.Arguments[0], prefix: "%", postfix: "%"));
                }

                if (methodCall.Method == typeof(string).GetMethod(nameof(string.StartsWith), new[] { typeof(string) }))
                {
                    return SqlExpression.Concat(Recurse(ref i, methodCall.Object), "LIKE", Recurse(ref i, methodCall.Arguments[0], postfix: "%"));
                }

                if (methodCall.Method == typeof(string).GetMethod(nameof(string.EndsWith), new[] { typeof(string) }))
                {
                    return SqlExpression.Concat(Recurse(ref i, methodCall.Object), "LIKE", Recurse(ref i, methodCall.Arguments[0], prefix: "%"));
                }
                ...
                if (methodCall.Method.Name == nameof(IList.Contains))
                {
                    ...
                    return SqlExpression.Concat(Recurse(ref i, property), "IN", SqlExpression.WithParameters(ref i, values));
                }
